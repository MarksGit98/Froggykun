...
func state_process(_delta):
	if _is_air_attack():
		...
		var has_floor_contact := character.is_on_floor()
		if character.has_method("has_grounded_attack_floor_contact"):
			has_floor_contact = character.call("has_grounded_attack_floor_contact")
		if has_floor_contact:
...
func state_input(event : InputEvent):
	if event.is_action_pressed("defend"):
		if defend_state != null and character.has_method("request_defend") and character.call("request_defend", AnimationConstants.PLAYER_STATE_ATTACKING):
...
func _refresh_active_animation() -> void:
	if _is_air_attack():
		active_animation = AnimationConstants.PLAYER_AIR_ATTACK
	else:
...
func _finish_air_attack() -> void:
	var source_state_name := StringName(character.call("get_air_attack_source_state_name"))
	...
	if source_state_name == AnimationConstants.PLAYER_STATE_JUMPING:
