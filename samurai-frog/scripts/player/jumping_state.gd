...
func state_process(_delta):
	if character.has_method("is_wall_jump_animation_requested") and character.call("is_wall_jump_animation_requested"):
		var current_animation: StringName = _get_current_animation_node()
		if current_animation == AnimationConstants.PLAYER_WALL_JUMP or current_animation == AnimationConstants.PLAYER_JUMP:
			character.call("clear_wall_jump_animation_request")
	if character.has_method("is_air_attack_return_jump_requested") and character.call("is_air_attack_return_jump_requested"):
		if _get_current_animation_node() == AnimationConstants.PLAYER_JUMP:
	...
	if can_reattach_to_wall:
		...
		if has_wall_contact:
			if character.velocity.y > 0.0:
				if character.has_method("try_snap_to_grounded_ledge_from_jump") and character.call("try_snap_to_grounded_ledge_from_jump"):
					if character.has_method("debug_attack_log"):
						character.call("debug_attack_log", "Jumping.state_process wall contact snapped to ledge -> Grounded")
					next_state = grounded_state
					return
				if character.has_method("debug_attack_log"):
					character.call("debug_attack_log", "Jumping.state_process wall contact while descending -> Walled")
...
func state_input(event : InputEvent):
	...
	if event.is_action_pressed("attack"):
		if character.has_method("request_air_attack") and character.call("request_air_attack", AnimationConstants.PLAYER_STATE_JUMPING):
	...
	if(event.is_action_pressed("jump")):
		...
		if character.has_method("debug_attack_log"):
			character.call("debug_attack_log", "Jumping.state_input jump pressed -> wall jump buffered")
